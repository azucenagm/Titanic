...
Se revisan los tipos asignados a cada variable: Survived, Sex y Embarked son variables categóricas por lo que se deben declarar como factores.
...
sum(duplicated(titanicData$PassengerId), na.rm=TRUE)
```
...
Revisamos qué registros son los que tienen que el importe de Fare a 0.
...
Respecto a la edad, vemos en el histograma que hay una acumulación de observaciones alrededor de edades intermedias (entre los 20 y 40 años aproximadamente). También puede observarse un sesgo a la derecha, hacia edades superiores.
...
corrplot(cor(numData, method="spearman"))
```
...
En cuanto al puerto de embarque, se aprecian ciertas diferencias, siendo más relevantes en el caso de Southampton, donde el porcentaje de supervivencia es menor en más de 10 puntos respecto al siguiente de los puertos. Quizá esta discrepancia se deba a otras características comunes que pudieran tener los pasajeros de un puerto u otro, por lo que vamos a investigar más detalladamente la relación de esta variable con otras.
...
colnames(pclassEmbarkedTable) <- c("Bajo", "Medio", "Alto")
...
Como se puede comprobar, el segundo modelo generado tiene un porcentaje ligeramente mayor de acierto, lo que implica que el uso de sólo esas tres variables le beneficia.
...
El error estimado (OOB error) de ambos modelos es muy parejo, por lo que se puede concluir que tienen un desempeño muy parejo. Seleccionamos el segundo de ellos y creamos una gráfica a continuación donde aparece:
...
# Construimos la curva ROC para el modelo con modelo de regresión logística
...
Según los valores de AUC obtenidos, el mejor de los modelos es el segundo que los que hemos construido con la técnica de random forest.
...
Mientras que para el modelo de random forest, las siguientes variables en importancia son Fare (coste del billete) y Age (edad del pasajero), para el modelo de regresión logística es Pclass (clase del pasaje). De hecho, la variable Fare fue eliminada en el segundo modelo de regresión construido al concluir que no era una variable significativa.
